...
function mostrardatos(datos) {
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            html += "<td><button class='btn btn-default glyphicon glyphicon-trash ' name='btn_eliminar'id='btn_eliminar'OnClick='eliminar_det_nota_credito(this)' type='button' value='" + registro[i]["id"] + "' title='ELiminar Item'></button></td>";
            $("#det_nota_credito").html(html);
            }
...
function guardar_det_producto(datos){
	event.preventDefault();
	$.ajax({
		url: "/factura/guardar_det_factura_item",
		type:"POST",
		data: $("#form-guardar").serialize(),
		success:function(respuesta){
		mostrardatos("");
...
function eliminar_det_nota_credito(boton){
	ID=boton.value;
	$.ajax({
		url: "eliminar_det_nota",
		type:"POST",
		data:{id:ID},
		success:function(respuesta){
			mostrardatos("");
...
function eliminar_nota_credito(boton){
	var respuesta = confirm("Desea Salir?");
	if(respuesta){
	ID=boton.value;
		$.ajax({
				url: "eliminar_toda_nota_credito",
				type:"POST",
				data:{id:ID},
				success:function(respuesta){
		    	top.location.href="nota_credito";//redirection
				}
		});
	}
}
