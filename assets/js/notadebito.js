...
function mostrardatos(datos) {
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            html += "<td><button class='btn btn-default glyphicon glyphicon-trash ' name='btn_eliminar'id='btn_eliminar'OnClick='eliminar_nota_debito(this)' type='button' value='" + registro[i]["id"] + "' title='ELiminar Item'></button></td>";
...
function eliminar_nota_debito(boton){
	ID=boton.value;
	$.ajax({
		url: "eliminar_det_nota_debito",
		type:"POST",
		data:{id:ID},
		success:function(respuesta){
			mostrardatos("");
...
function eliminar_nota_debito_salir(boton){
	var respuesta = confirm("Desea Salir?");
	if(respuesta){
	ID=boton.value;
		$.ajax({
				url: "eliminar_toda_nota_debito",
				type:"POST",
				data:{id:ID},
				success:function(respuesta){
		    	top.location.href="nota_debito";//redirection
				}
		});
	}
}
