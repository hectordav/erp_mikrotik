...
function mostrardatos(datos) {
    $.ajax({
        ...
        success: function(respuesta) {
            ...
            html += "<td><button class='btn btn-default glyphicon glyphicon-trash ' name='btn_eliminar'id='btn_eliminar'OnClick='eliminar_det_nota_venta(this)' type='button' value='" + registro[i]["id"] + "' title='ELiminar Item'></button></td>";
            $("#det_nota_venta").html(html);
...
function guardar_det_producto_nota_venta(datos){
	event.preventDefault();
	$.ajax({
		url: "guardar_det_nota_venta_item",
		type:"POST",
		data: $("#form-guardar").serialize(),
		success:function(respuesta){
		mostrardatos("");
...
function eliminar_det_nota_venta(boton){
	ID=boton.value;
	$.ajax({
		url: "/nota_venta/eliminar_det_nota_venta",
		type:"POST",
		data:{id:ID},
		success:function(respuesta){
			mostrardatos("");
...
function eliminar_nota_venta(boton){
	var respuesta = confirm("Desea Salir?");
	if(respuesta){
	ID=boton.value;
		$.ajax({
				url: "/nota_venta/eliminar_toda_nota_venta",
				type:"POST",
				data:{id:ID},
				success:function(respuesta){
		    	top.location.href="/nota_venta";//redirection
				}
		});
	}
}
